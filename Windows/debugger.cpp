    ...
    while (cur_code > min_address)
    {
        ...
        if (meminfobuf.State == MEM_FREE)
        {
            if (meminfobuf.RegionSize >= size)
            {
                ...
                if (ret_address)
                {
                    if (((size_t)ret_address >= min_address) && ((size_t)ret_address <= max_address))
                    {
                        return ret_address;
                    }
                    else
...
void Debugger::GetExceptionHandlers(size_t module_haeder, std::unordered_set<size_t> &handlers)
{
    ...
    char *modulebuf = (char *)malloc(size_of_image);
    SIZE_T num_read;
    if (!ReadProcessMemory(child_handle, (void *)module_haeder, modulebuf, size_of_image, &num_read) || (num_read != size_of_image))
    {
        FATAL("Error reading target memory\n");
    }
...
DebuggerStatus Debugger::HandleExceptionInternal(EXCEPTION_RECORD *exception_record)
{
    ...
    if ((exception_record->ExceptionCode == EXCEPTION_BREAKPOINT) || (exception_record->ExceptionCode == 0x4000001f))
    {
    ...
    case EXCEPTION_ACCESS_VIOLATION: {
        if (target_function_defined && ((size_t)exception_record->ExceptionAddress == PERSIST_END_EXCEPTION))
        {
