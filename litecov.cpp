...
ModuleCovData *LiteCov::GetDataByRemoteAddress(size_t address)
{
    for (ModuleInfo *module : instrumented_modules)
    {
        ...
        if ((address >= (size_t)data->coverage_buffer_remote) && (address < ((size_t)data->coverage_buffer_remote + data->coverage_buffer_size)))
        {
            return data;
        }
    }
...
{
    if ((exception_record->type == ACCESS_VIOLATION) && (exception_record->maybe_write_violation))
    {
        ModuleCovData *data =
